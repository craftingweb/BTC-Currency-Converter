...
function App() {
	...
	return (
		<div className="App">
			...
			<div className="main">
				{isCurrentRates ? (
					<CurrentRates currencyData={currency} />
				) : (
					<Conversions />
				)}
			</div>
...
const Conversions = () => {
	...
	function handleConvert() {
		...
		const valueInBtc = (amount * (1 / currencyObj[currency].rate_float)).toFixed(
			3
		);
	...
	const sortedExchangeRates = sortExchangeRates(
		[
			{
				currency: 'USD',
				rate: currencyObj && currencyObj.USD.rate,
				rate_float: currencyObj && currencyObj.USD.rate_float,
			},
			{
				currency: 'EUR',
				rate: currencyObj && currencyObj.EUR.rate,
				rate_float: currencyObj && currencyObj.EUR.rate_float,
			},
			{
				currency: 'GBP',
				rate: currencyObj && currencyObj.GBP.rate,
				rate_float: currencyObj && currencyObj.GBP.rate_float,
			},
		],
		sortOrder
	);
	...
	return (
		<div>
			<div className="convert-currency-container">
				...
				<h2>
					{amount} {currency} is equal to {btcValue} BTC
				</h2>
			</div>
			...
			<div>
				...
				{sortedExchangeRates.map((exchangeRate) => (
					<li key={exchangeRate.currency}>
						{exchangeRate.currency} to BTC: {exchangeRate.rate} (
						{exchangeRate.rate_float})
					</li>
				))}
			</div>
		</div>
